...
If the user says to use Browser Use but the task can be handled with the existing real browser path, prefer **browser real** first.

Practical rule:
- Default to **browser real** for ordinary browsing / logged-in local browsing / local tab control.
- Use **Browser Use cloud** only when its cloud session capability is specifically needed, such as remote cloud browser sessions, Browser Use profiles, Browser Use autonomous tasks, or when the user explicitly wants Browser Use itself.
